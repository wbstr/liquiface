 ...
 * @author tveki
 */
@ApplyChange(DropUniqueConstraintChange.class)
public class DropUniqueConstraintChangeApplier implements ChangeApplier{

    @Override
    public void apply(Model model, Change c) {
        DropUniqueConstraintChange change = (DropUniqueConstraintChange) c;
        Table table = model.getTableByName(change.getTableName());
        UniqueConstraint constraint = table.getUniqueConstraintByName(change.getConstraintName());        
