 ...
 * @author tveki
 */
@ApplyChange(ModifyDataTypeChange.class)
public class ModifyDataTypeChangeApplier implements ChangeApplier{

    @Override
    public void apply(Model model, Change c) {
        ModifyDataTypeChange change = (ModifyDataTypeChange) c;
        Table table = model.getTableByName(change.getTableName());
        Column column = table.getColumnByName(change.getColumnName());
        String newDataType = change.getNewDataType();
        column.setColumnType(new ColumnType(newDataType));
    }
