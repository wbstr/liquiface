 ...
 * @author tveki
 */
@ApplyChange(AddPrimaryKeyChange.class)
public class AddPrimaryKeyChangeApplier implements ChangeApplier {

    @Override
    public void apply(Model model, Change c) {
        AddPrimaryKeyChange change = (AddPrimaryKeyChange) c;
        
        Table table = model.getTableByName(change.getTableName());
        
        PrimaryKeyConstraint constraint = PrimaryKeyConstraintFactory
                        .createPrimaryKeyConstraint(change.getConstraintName(),
                                                    getPrimaryKeyColumns(table, change));
        
        table.setPrimaryKeyConstraint(constraint);
    }    
    
    private List<Column> getPrimaryKeyColumns(Table table, AddPrimaryKeyChange change){
