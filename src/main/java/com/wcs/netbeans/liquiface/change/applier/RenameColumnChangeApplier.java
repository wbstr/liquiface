 ...
 * @author tveki
 */
@ApplyChange(RenameColumnChange.class)
public class RenameColumnChangeApplier implements ChangeApplier{

    @Override
    public void apply(Model model, Change c) {
        RenameColumnChange change = (RenameColumnChange) c;
        Table table = model.getTableByName(change.getTableName());
        Column column = table.getColumnByName(change.getOldColumnName());
        column.setName(change.getNewColumnName());
    }
