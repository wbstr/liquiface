 ...
 * @author tveki
 */
public class PrimaryKeyConstraintFactory {
    
    public static PrimaryKeyConstraint createPrimaryKeyConstraint(String name, List<Column> primaryKeyColumns){
        PrimaryKeyConstraint constraint = new PrimaryKeyConstraint(name);
        for (Column pkColumn : primaryKeyColumns){
            pkColumn.setPrimaryKey(true);
            pkColumn.setNullable(false);        
            constraint.addColumn(pkColumn);
        }        
        return constraint;
    }           
